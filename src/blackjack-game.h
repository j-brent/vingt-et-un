...
namespace CardGames::BlackJack
{
	...
	public:
		...
		[[nodiscard]] const std::vector<Card>& cards() const { return m_cards; }
		void add(const Card& card) { m_cards.push_back(card); }

		[[nodiscard]] HandValue value() const { return calculate_hand_value(m_cards); }
		[[nodiscard]] int total() const { return value().total; }
		[[nodiscard]] bool is_soft() const { return value().is_soft; }
	...
	public:
		...
		[[nodiscard]] const std::vector<Card>& active_cards() const
		{
			return m_hands[m_active_index].cards;
		}
		void add_to_active(const Card& card) { m_hands[m_active_index].cards.push_back(card); }
		[[nodiscard]] int active_total() const { return calculate_hand_value(active_cards()).total; }
		[[nodiscard]] bool active_is_busted() const { return active_total() > 21; }

		// Split operations
		[[nodiscard]] bool can_split(bool allow_resplit_aces = false) const
		{
		...
		void split(const Card& first_new_card,
							 const Card& second_new_card) // NOLINT(bugprone-easily-swappable-parameters)
		{
		...
		[[nodiscard]] bool all_busted() const
		{
		...
		// Multi-hand queries
		[[nodiscard]] size_t hand_count() const { return m_hands.size(); }
		[[nodiscard]] size_t active_index() const { return m_active_index; }
		[[nodiscard]] const std::vector<SingleHand>& all_hands() const { return m_hands; }
	...
	enum class GameNode : std::uint8_t {
		Ready,
		PlayersRound,
	...
	public:
		...
		[[nodiscard]] GameNode node() const { return m_node; }
		[[nodiscard]] const PlayersHand& players_hand() const { return m_players_hand; }
		[[nodiscard]] const DealersHand& dealer_hand() const { return m_dealers_hand; }
		[[nodiscard]] const Deck& deck() const { return m_deck; }
	...
	{
	public:
		enum class Play : std::uint8_t { Deal, Hit, Stay, Split };

		explicit Game(BlackjackConfig config = {})
			: m_config{config}
			, history{config.initial_deck
									? std::vector<GameState>{GameState{GameNode::Ready, PlayersHand{}, DealersHand{},
																										 *config.initial_deck}}
		...
		[[nodiscard]] const GameState& state() const { return history.back(); }
